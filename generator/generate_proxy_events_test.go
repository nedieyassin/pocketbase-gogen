...
func TestProxyEventGeneration(t *testing.T) {
	expectedGeneration := `type baseProxyEventData[P Proxy, PP ProxyP[P]] struct {
	PRecord PP
}
...
`
	...
	if !expectGeneratedEvents(expectedGeneration) {
		t.Fatal("the generated events code differs from the expected result")
	}
}

func expectGeneratedEvents(expectedOutput string) bool {
	outBytes, err := GenerateProxyEvents(".", "test")
	if err != nil {
		return false
	}
